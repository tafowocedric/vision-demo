...
cmake -DCMAKE_BUILD_TYPE=RELEASE -DOPENCV_GENERATE_PKGCONFIG=YES -DCMAKE_INSTALL_PREFIX=/usr/local \
      -DWITH_V4L=ON -DWITH_EIGEN=ON -DBUILD_EXAMPLES=ON -DBUILD_PERF_TESTS=OFF -DOPENCV_ENABLE_NONFREE=ON \
      -DINSTALL_C_EXAMPLES=ON -DBUILD_opencv_python3=ON \
